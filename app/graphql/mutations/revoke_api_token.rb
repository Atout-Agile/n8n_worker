...
# Revocation is immediate — the token's expiration is set to the current time.
#
...
module Mutations
  ...
  #
  # @!method resolve(id:)
  #   Immediately revokes the token by setting its expiration to now
  #   @param id [ID] The ID of the token to revoke (required)
  #   @return [Hash] Hash containing :success and :errors
  #     - :success [Boolean] True if the token was revoked successfully
  ...
  class RevokeApiToken < GraphQL::Schema::Mutation
    # @!attribute [r] id
    ...
    # Resolves the mutation by revoking the specified API token
    #
    ...
    def resolve(id:)
      ...
      return { success: false, errors: ['You must be logged in to revoke an API token'] } if current_user.nil?
